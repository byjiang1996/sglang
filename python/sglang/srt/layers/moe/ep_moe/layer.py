...
class EPMoE(torch.nn.Module):
    ...
    def forward_normal(self, hidden_states: torch.Tensor, router_logits: torch.Tensor):
        ...
        # topk_ids = [[0, 3, 4], [0, 2, 4]]
        # reorder_topk_ids = [0, 0, 2, 3, 4, 4]
        # reorder_ids = [0, 3, 4, 1, 2, 5]
        # seg_indptr = [0, 2, 2, 3, 4, 6]; start index of each expert_id in reorder_topk_ids
        # src2dst = [0, 3, 4, 1, 2, 5]; source = reorder_topk_ids's index; dest = topk_ids's index
        # gateup_input is generated by topk_ids
        # gateup_input will be used together with reorder_topk_ids
        # gateup_input[0] = hidden_states[0]
        # gateup_input[1] = hidden_states[1]
        # gateup_input[2] = hidden_states[1]
        # gateup_input[3] = hidden_states[0]
        # gateup_input[4] = hidden_states[0]
        # gateup_input[5] = hidden_states[1]
        ...
        # For each expert in the current partition:
        # gateup_output[expert_start:expert_end] = gateup_input[expert_start:expert_end] @ w13_weight[expert_id].T
        # GroupGemm-0
        ...
        # For each expert in the current partition:
        # down_output[expert_start:expert_end] = down_input[expert_start:expert_end] @ w2_weight[expert_id].T
